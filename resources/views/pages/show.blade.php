<x-app-layout>
    <main class="py-8 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700 text-gray-900 dark:text-gray-100">

        <div class="max-w-7xl mx-auto sm:p-6 lg:px-8 bg-white dark:bg-slate-900  dark:border-slate-700">
            @php
                $post = $post ?? ($job ?? null);
            @endphp

            @if (!$post)
                <div class="bg-white dark:bg-slate-900 dark:border-slate-700 shadow-sm rounded-lg p-6">
            ...
            @else
                <div class="md:grid grid-cols-12 gap-4 dark:bg-slate-900 ">

                    <div class="col-span-8 bg-white dark:bg-slate-900 p-4 rounded-lg shadow">

                        <!-- Job Description -->

                       <div
    class="bg-white dark:bg-slate-900 rounded-xl mb-5 shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden p-6 relative">
        ...
        <div class="mt-1">
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach ($benefitsParts as $benefit)
                    <div
                        class="flex items-center p-3 bg-gray-50 dark:bg-slate-700 rounded-lg border border-gray-200 dark:border-slate-600">
                        ...
                        <span class="ml-3 text-sm text-gray-600 dark:text-gray-300">{{ $benefit }}</span>
                    </div>
                @endforeach
            </div>
        </div>
...
<div
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach ((array) $post->qualifications as $qualification)
                    <span
                        ...
                        <svg class="w-4 h-4 mr-2 text-purple-600 dark:text-purple-300"
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach ($post->skills as $skill)
                    <span
                        ...
                        <svg class="w-4 h-4 mr-2 text-blue-600 dark:text-blue-300" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            @if ($post->created_at)
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $post->created_at->format('M d, Y') }}
                        </p>
                    </div>
                </div>
            @endif
            ...
            @if ($post->updated_at && $post->updated_at->ne($post->created_at))
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ __('Last Updated') }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
    ...
    <div class="mt-10">
        <h3 class="text-2xl font-semibold mb-6 text-gray-900 dark:text-gray-100 flex items-center">
            <svg class="h-6 w-6 mr-2 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @auth
            <form action="{{ route('comments.store', $job) }}" method="POST"
                class="mb-6 bg-gray-50 dark:bg-slate-900 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
                @csrf
                <textarea name="content" rows="3"
                    class="w-full rounded-md border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900
                           text-gray-900 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm resize-none"
        ...
        @else
            <p class="text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-slate-900 p-3 rounded-lg">
        ...
        <div class="space-y-6 max-h-80 overflow-auto">
            @foreach ($job->comments()->whereNull('parent_id')->latest()->get() as $comment)
                <div x-data="{ openReply: false }"
                    class="border border-gray-200 dark:border-slate-700 p-3 rounded-md bg-white dark:bg-slate-900">
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-start">
                                <div>
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-gray-100">{{ $comment->user->name }}</span>
                            ...
                            <div class="mt-1 text-gray-900 dark:text-gray-200">{{ $comment->content }}</div>
                            ...
                            @auth
                                ...
                                <form x-show="openReply" action="{{ route('comments.store', $job) }}" method="POST"
                                    class="mt-2">
                                    @csrf
                            ...
                            @if ($comment->replies->count())
                                <div class="mt-3 ml-6 space-y-3 border-l border-gray-200 dark:border-slate-700 pl-4">
                                    @foreach ($comment->replies as $reply)
                                        <div class="flex items-start space-x-3 text-sm">
                                            ...
                                            <div class="flex-1">
                                                <div class="flex justify-between items-start">
                                                    <span
                                                        class="font-medium text-gray-700 dark:text-gray-100">{{ $reply->user->name }}</span>
                                                ...
                                                <div class="text-gray-900 dark:text-gray-200">{{ $reply->content }}</div>
    ...
    class="bg-white dark:bg-slate-900 h-[25rem] relative md:col-span-4 rounded-xl shadow-md border border-gray-200 dark:border-slate-700 p-6 hover:shadow-lg transition-all duration-200">
            ...
            {{ $s === 'published'
                ...
                {{ $s === 'published'
                    ...
                    : ($s === 'draft'
                        ...
                        <!-- Company Logo -->
<div
    class="w-24 h-24 mx-auto mb-5 rounded-full overflow-hidden bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 flex items-center justify-center">
