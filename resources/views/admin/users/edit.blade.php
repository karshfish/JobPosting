...
@section('page-actions')
  <div class="flex items-center gap-2">
    ...
    @if(auth()->id() !== $user->id)
      <form method="POST" action="{{ route('admin.users.destroy', $user) }}" onsubmit="return confirm('Deactivate this user? You can activate them again later.');">
        @csrf @method('DELETE')
        <button class="inline-flex items-center gap-2 px-3 py-1.5 rounded-md border border-red-300 text-red-700 hover:bg-red-50 dark:hover:bg-red-900/30 transition-base text-sm">Deactivate</button>
      </form>
    @endif
  </div>
@endsection
