...
@section('content')
  <h1 class="text-xl font-semibold text-slate-800 mb-4">Edit Category</h1>
  <form method="POST" action="{{ route('admin.categories.update',$category) }}" class="space-y-4">
    @csrf @method('PUT')
    <div>
      <label class="block text-sm font-medium text-slate-700 mb-1">Name</label>
      <input class="block w-full rounded-md border-slate-300 focus:border-blue-400 focus:ring-blue-400" name="name" value="{{ old('name', $category->name) }}" required>
      @error('name')
