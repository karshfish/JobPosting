<x-candidate-layout>
    <div class="container mx-auto px-6 py-10">
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 shadow-lg border border-gray-100 dark:border-gray-700 rounded-2xl p-8 hover:shadow-2xl transition">
            ...
            <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-6 text-center border-b dark:border-gray-600 pb-4">
                Edit Application for: <span class="text-blue-600 dark:text-blue-400">{{ $application->job->title }}</span>
            </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 text-gray-700 dark:text-gray-300">
                ...
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border dark:border-gray-600">
                    ...
                    <p class="capitalize">{{ $application->job->work_type ?? 'N/A' }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border dark:border-gray-600">
                    <p class="font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-1 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <p>{{ $application->job->application_deadline ?? 'N/A' }}</p>
                </div>
            </div>
            ...
            <div class="mb-10">
                ...
                <p class="text-gray-600 dark:text-gray-400 leading-relaxed">{{ $application->job->description }}</p>
            </div>

            {{-- Candidate Edit Form --}}
            <form action="{{ route('candidate.applications.update', $application) }}" method="POST" enctype="multipart/form-data"
                class="space-y-6" id="editApplicationForm">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $application->name ?? $user->name ?? '') }}"
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $application->email ?? $user->email ?? '') }}"
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $application->phone ?? $user->phone ?? '') }}"
                ...
                <div>
                    ...
                    <input type="file" name="resume" accept="application/pdf" id="resumeEditInput"
    ...
    <script>
        document.getElementById('editApplicationForm').addEventListener('submit', function(e) {
            const fileInput = document.getElementById('resumeEditInput');
